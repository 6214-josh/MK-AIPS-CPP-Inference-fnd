...
export default function RunCardPanel(){
  ...
  async function loadDqnActions(){ setSuggestions((await apiClient.get('/run-cards/dqn/actions')).data||[]) }
  ...
  async function generateDqn(){ const res=await apiClient.post('/run-cards/dqn/suggest'); setResult(`已產生 ${res.data.created||0} 筆 DQN 建議`); await loadDqnActions() }
  useEffect(()=>{load()},[])
