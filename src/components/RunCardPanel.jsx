...
export default function RunCardPanel(){
  ...
  const [aiStatus,setAiStatus]=useState({})
    const [form,
    setForm]=useState({run_card_no:'MK20240069',
  ...
  const headerColumns=['run_card_id','run_card_no','production_batch_no','work_order_no','customer_name','product_no','material_no','planned_qty','remaining_qty','due_date','priority_level','run_card_status']
    const detailColumns=['sequence_no',
    'station_name',
    'station_sub_name',
    'process_type',
    'cnc_machine_id',
  ...
  async function load(){ const h=(await apiClient.get('/run-cards/headers')).data||[];
  setHeaders(h);
  setFeatures((await apiClient.get('/run-cards/features')).data||[]);
  await loadDqnActions();
  await loadAiStatus();
  if(h.length && !selectedRunCardId){ setSelectedRunCardId(String(h[0].run_card_id));
  await loadDetail(h[0].run_card_id) } }
  ...
  const statusText = `PyTorch=${aiStatus.pytorch ? '已啟用' : '未啟用'}${
    ...
    aiStatus.statsmodels_version ? ` (${aiStatus.statsmodels_version})` : ''
  }`
  ...
  <button onClick={generateDqn}>產生 DQN 建議</button>
  ...
  </div>{result&&<div className="export-message">操作結果：{result}
  </div>}<div className="metric-grid">
  <div className="metric-card">
  <div className="metric-label">流程卡單頭</div>
  <div className="metric-value">{headers.length}
  </div>
  <div className="metric-hint">AIPS_RUN_CARD_HEADER</div>
  </div>
  <div className="metric-card">
  <div className="metric-label">AI 特徵</div>
  <div className="metric-value">{features.length}
  </div>
  <div className="metric-hint">LSTM / ARIMA Feature</div>
  </div>
  <div className="metric-card">
  <div className="metric-label">DQN 建議</div>
  <div className="metric-value">{dqnActionTotal}
  </div>
  <div className="metric-hint">Action Suggestion（表格顯示最近 {suggestions.length} 筆）</div>
  </div>
  </div>
  <div className="card">
  <h2>新增流程卡單頭</h2>
  <div className="form-grid">{fields.map(([k,l])=>
  <label key={k}>{l}<input value={form[k]} onChange={e=>update(k,e.target.value)} />
  </label>)}
  </div>
  <button className="primary-btn" onClick={createHeader}>新增單頭</button>
  </div>
  <div className="card">
  <h2>流程卡單頭</h2>
