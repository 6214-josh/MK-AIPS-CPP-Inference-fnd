...
export default function MeterDashboard() {
  ...
  <div className="electric-dashboard">
    ...
    ' kg',
    'blueBlock color-down']].map(x=>
  <div className={`electric-card ${x[3]}`} key={x[0]}>
  ...
  <div className="electric-top">{[['即時用電量',data.p,' kW','greenBlock'],['最大需量',data.pdm,' kW','greenBlock'],['THD 電流',data.thdCurrent,' %',Number(data.thdCurrent||0)>=15?'dangerBlock':''],['功率因數',data.pf,'','']].map(x=>
  <div className={`electric-card ${x[3]}`} key={x[0]}>
  ...
  <div className="bar-chart">{(current.monthlyDataList||[]).map(row=>
  <div className="bar-item" key={row.monthString}>
