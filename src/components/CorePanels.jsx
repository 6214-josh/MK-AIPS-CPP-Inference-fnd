...
export function Dashboard() {
  ...
  function actionClass(a){ const name=a.action_name||'', reason=a.action_reason||'';
  ...
  <PageHeader title="AIPS / MK-AIPS 智慧排程總覽" subtitle="整合 CNC 智慧電表、ERP 製令單、WMS 線邊庫資料，建立 DQN State，產生排程建議與 Reward 回饋。">
  <button className="primary-btn" onClick={runFullFlow}>一鍵執行 AIPS 流程</button>
  <button onClick={load}>重新整理</button>
  </PageHeader>{flowMessage&&<div className="export-message">操作結果：{flowMessage}
  ...
  <div className="metric-value">{x[1]||0}
  </div>
  ...
  <div className="card">
  <h2>AI 排程流程</h2>
  <div className="flow-grid">{['資料收集|CNC 電力、ERP 製令單、WMS 線邊庫庫存。','特徵工程|計算功率、THD、機台狀態、缺料風險。','DQN State|整合設備、工單、庫存、交期、OEE 狀態。','排程建議|提前補料、提高優先權、換機、預防保養。'].map((s,i)=>{const [a,b]=s.split('|');
